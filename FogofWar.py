...
class FogofWar():
  ...
  def determine_vision(vision_radius : int, starting_coord : (int, int), map : [[str]] ) -> [(int, int)]:
    ...
    queueList = [(starting_coord[0],starting_coord[1] - 1), (starting_coord[0],starting_coord[1] + 1), (starting_coord[0] - 1,starting_coord[1]), (starting_coord[0] + 1,starting_coord[1])]
    visibleTiles = [starting_coord]
    while len(queueList) > 0:
      cell = queueList.pop(0)
      visibleTiles.append(cell)
      if FogofWar.taxicab_distance(starting_coord, cell) < vision_radius and FogofWar.is_cell_transparent(cell, map):
        neighboring_cells = FogofWar.neighbor_cells(starting_coord, cell, map)
        for cell in neighboring_cells:
          if cell not in queueList and cell not in visibleTiles:
            queueList.append(cell)
    return visibleTiles
